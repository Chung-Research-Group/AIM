...
AIM modules **IsoFit** and **HeatFit** read isotherm data from input files and generate output files containing
...
Sample files are available in the repository's
`input-files directory <https://github.com/Chung-Research-Group/AIM/tree/main/input-files>`_.
Copy a representative file and replace its data while preserving the metadata
tags for a quick start with **IsoFit** or **HeatFit**.

Output files
------------

**IsoFit** and **HeatFit** generate an output containing the isotherm model name, fitted parameters, and fitting statistics.
A typical output file from **HeatFit** is shown below.
