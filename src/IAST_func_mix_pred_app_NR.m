function partial_loadings = IAST_func_mix_pred_app_NR(num_components, isotherm_params_array, Pressure, gas_phase_mol_fraction, T, ~)
    %% Parameter Unpacking and Decalarations
    size_of_pressure_vector = length(Pressure(:, 1));
    partial_pressures = Pressure .* gas_phase_mol_fraction;
    fictitious_pressure = zeros(size_of_pressure_vector, num_components);
    T_array = ones(size_of_pressure_vector, 1) .* T;
    T_flag = 0;
    %
    ...
    p_comp = repmat(p_range, 1, num_components);
    loading = Isotherm_functions(num_components, isotherm_params_array, p_comp, 298.15, T_flag, 1);
        ...
        try
            ...
            if sum(partial_pressures(k, :), 2) <= tolerance
                ...
                elseif isscalar(idx)
                    ...
                    solution = NR_function(H_guess(k, idx), isotherm_params_array(:, idx), ...
                                           partial_pressures(k, idx), T_array(k, 1), length(idx),...
                                           T_flag, max_iter);
    ...
    function converged_sol = NR_function(p0_guess, isotherm_params_array, partial_pressure, T, ncomp, T_flag, max_iter)
        ...
        for m=1:max_iter
            ...
            if (std(grand_potentials) < convergence_tol) && (mol_frac_check || iter_count > 100)
                convergence_flag = 1;
                break;
