function derivatives = Adsorption_step_EDSL_Non_Iso(~, state_vars, Params, isotherm_params_array, comp_num)
...
%% Boundary Conditions
    ...
    elseif Params(end) == 0
        ...
        % vh(1) = 1/epsilon;
        % P(1) = ((viscous_term.*vh(1)*v_0*dz/2*L./P_0)+P(2))./(1-(dz/2*L/R/T(1)/T_0)*MW_t(1)*(1.75*(1-epsilon)/2/r_p/epsilon)*vh(1).^2.*v_0.^2); 
    else
...
%% Adsoption Term
    ...
    eq_loading(:, 1:comp_num-1) = EDSL_func(comp_num-1, P.*P_0, y_array(:, 1:comp_num-1), T.*T_0, isotherm_params_array, 1);
%% Temporal Derivatives
...
%%   2.3) Calculate the temperature change due to adsorption/desoprtion enthalpy  
    ...
    dTdt3(2:N+1)      = -1.0 .* (ro_s .* q_s0 ./ T_0) .* (generation_term_1.* dx1dt(2:N+1) +...
