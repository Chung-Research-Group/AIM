...
In IsoFit, regression is performed using MATLAB's built-in nonlinear least-squares solver **lsqnonlin**. The user can control the regression process by specifying custom initial guesses, as well as lower and upper bounds.
...
.. math::
  r^{2} = 1 - \frac{SSE}{\sum_i (q_{i,exp} - \overline{q_{exp}})^2}

where :math:`\overline{q_{exp}}` is the mean experimental gas uptake.
