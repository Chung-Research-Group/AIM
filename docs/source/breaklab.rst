...
**BreakLab** is the module for isothermal/non-isothermal multicomponent fixed bed breakthrough simulation for up to 5 components (1 non-adsorbing, 4
adsorbing components).
**BreakLab** GUI is shown below:

.. image:: _static/BreakLab.png
   :width: 1000
   :alt: BreakLab_logo
   :align: center
...
of the system. The mathematical model of BreakLab including the balance equations, LDF, and Ergun equation is summarized in the table below. Please check the journal article for a more detailed discussion on BreakLab mathematical model.
...
.. list-table:: BreakLab Balance Equations
   ...
   * - Wall energy balance
     - :math:`\rho_{wall}C_{wall}\frac{\partial T_{wall}}{\partial t} = K_{z,wall} \frac{\partial^{2}T_{wall}}{\partial z^2} - \frac{2 h_{in} r_{in}}{(r_{out}^{2}-r_{in}^{2})}(T_{wall} - T) - \frac{2 h_{out} r_{out}}{(r_{out}^{2}-r_{in}^{2})}(T_{wall} - T_{a})`
...
Total porosity :math:`\varepsilon_{t}` is calculated using the bulk and particle porosities, :math:`\varepsilon_{b}`, :math:`\varepsilon_{p}`,
...
.. list-table:: BreakLab Boundary Conditions
   ...
   * - Pressure (:math:`P`)
     ...
     - :math:`P|_{z=L} = P_{0}`
   * - Wall temperature (:math:`T_{wall}`)
     - :math:`T_{wall}|_{z=0} = T_{a}`
     - :math:`T_{wall}|_{z=L} = T_{a}`
...
.. list-table:: BreakLab Inputs
   ...
   * - :math:`\varepsilon_{p}`
     ...
     - :math:`-`
     - In case of not knowing the particle porosity specify this as :math:`0.0`. In such case, BreakLab will assume bulk porosity :math:`\varepsilon_{b}` is equal to total porosity :math:`\varepsilon_{t}`.
   * - :math:`*C_{p,ads}`
...
Note that parameters with :math:`*` are only required for non-isothermal breakthrough simulation.

Nomenclature
-------------

| :math:`P =` Absolute pressure of the gas :math:`(Pa)`
| :math:`\overline{P} =` Dimensionless pressure of gas :math:`(-)`
| :math:`T =` Absolute temperature of the gas :math:`(K)`
| :math:`\overline{T} =` Dimensionless temperature of gas :math:`(-)`
| :math:`T_{wall} =` Wall temperature :math:`(K)`
| :math:`\overline{T_{wall}} =` Dimensionless wall temperature :math:`(-)`
| :math:`y_{i} =` Gas mole fraction of component :math:`i` :math:`(Pa)`
...
| :math:`q_{i}^{*} =` Mixture equilibrium adsorption loading of component :math:`i`  :math:`(mol/kg)`
...
| :math:`T_{0} =` Temperature of feed gas :math:`(K)`
| :math:`y_{0, i} =` Mole fraction of component :math:`i` in feed :math:`(-)`
| :math:`\rho_{b, ads} =` Bulk density of adsorbent bed :math:`(kg/m^{3})`
| :math:`\rho_{gas} =` Density of gas :math:`(kg/m^{3})`
| :math:`\rho_{wall} =` Density of wall :math:`(kg/m^{3})`
...
| :math:`D_{col,in} =` Inside diameter of adsorber column :math:`(m)`
| :math:`D_{col,out} =` Outside diameter of adsorber column :math:`(m)`
| :math:`\mu =` Viscosity of gas phase :math:`(Pa . s)`
| :math:`K_{z} =` Thermal conductivity of gas :math:`(W/m/K)`
| :math:`K_{z,wall} =` Thermal conductivity of wall :math:`(W/m/K)`
| :math:`C_{p,ads} =` Heat capacity of adsorbent particles :math:`(J/kg/K)`
| :math:`C_{p,a} =` Heat capacity of adsorbed phase :math:`(J/mol/K)`
| :math:`C_{p,gas} =` Heat capacity of gas :math:`(J/mol/K)`
| :math:`C_{p,wall} =` Heat capacity of wall :math:`(J/mol/K)`
| :math:`\Delta H_{ads, i} =` Heat of adsorption of component :math:`i` :math:`(J/mol)`
| :math:`T_{ref} =` Reference temperature at which isotherm parameters are obtained :math:`(K)`
| :math:`T_{a} =` Surroundings or ambient temperature :math:`(K)`
| :math:`h_{in} =` Inside heat transfer coefficient :math:`(W/m^{2}/K)`
| :math:`h_{out} =` Outside heat transfer coefficient :math:`(W/m^{2}/K)`
