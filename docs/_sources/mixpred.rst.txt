...
The solution of IAST involves nonlinear equations expressed through the reduced grand potential. For component :math:`i`, the reduced grand potential :math:`\psi_{i}^{*}` is
...
In **MixPred**, the IAST equations above are solved for :math:`2N` unknowns, :math:`P_{i}^{*}` and :math:`x_{i}`. The mixture adsorption loading for each component is then calculated using
